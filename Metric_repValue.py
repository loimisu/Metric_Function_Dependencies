...
for agroup in mf_pair_grouped:
    if len(agroup) > 2:
        ...
        # print agroup[0]
        header = agroup[0][0]
        flight_info = map(lambda item: time_to_str(item), agroup[0][1:])
        theta = 0.0
        flight_info.insert(3,theta)
        flight_info.insert(0,header)
        print flight_info
    else: #(if len(agroup) == 2 )
        header = agroup[0][0]
        x = max([agroup[0][1], agroup[1][1]])
        y = max([agroup[0][2], agroup[1][2]])
        theta = 0.0
        flight_info = [header,time_to_str(x),time_to_str(y),theta]
        print flight_info
